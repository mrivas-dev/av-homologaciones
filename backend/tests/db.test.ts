...
import { getClient, initializeDatabase, closeConnection } from "../config/db.ts";
...
Deno.test({
  ...
  async fn(t) {
    await t.step("should enforce user foreign key in homologations", async () => {
      ...
      try {
        ...
        errorThrown = true;
        console.log("✅ Correctly caught foreign key violation:", error.message);
      }
      
      assertEquals(errorThrown, true, "Should throw error for invalid foreign key");
    });
  },
  sanitizeResources: false,
  sanitizeOps: false,
...
async function cleanupTestData() {
  ...
  try {
    ...
    await client.execute("DELETE FROM photos WHERE created_by = ?", [testUserId]);
    await client.execute("DELETE FROM homologations WHERE created_by = ?", [testUserId]);
    await client.execute("DELETE FROM users WHERE username = ?", [testUser.username]);
